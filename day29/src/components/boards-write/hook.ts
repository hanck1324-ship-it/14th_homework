...
export const useBoardWrite = (isEdit: boolean) => {
  ...
  const [modalContents, setModalContents ] = useState(""); // 모달에 어떤 메시지를 보여줄지 보관하는 modalContents라는 상태를 만든 것 //
  const [isAlertModalOpen, setIsAlertModalOpen ] = useState(false); // 알림 모달 상태 추가
  ...
  const handleTogglePostcodeModal = () => setIsPostcodeModalOpen((prev) => !prev);
  ...
  return {
    ...
    isPostcodeModalOpen, isAlertModalOpen, modalContents, 
    data, isActive,
